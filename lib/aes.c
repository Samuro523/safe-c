...
const int MAXBC     = (256/32);
const int MAXKC     = (256/32);
...
/* AES lookup tables for both normal and optimized versions */
...
void fast_compute_key (    byte[] key,
                       out KEY    k)
{
  ref byte W[MAXROUNDS+1][4][4] = k.rk;
  int  nb_rounds, kc, j, r, t, rconpointer;
  byte tk[MAXKC][4];

  assert (key'length == 16 | key'length == 24 | key'length == 32);

  nb_rounds = (key'length >> 2) + 6;

  clear k;
  k.nb_rounds = nb_rounds;

  kc = nb_rounds - 6;

  clear tk;
  for (j=kc-1; j>=0; j--)
    *((uint4 *)&tk[j]) = *((uint4 *)&key[j<<2]);

  r = 0;
  t = 0;

  // copy values into round key array
  for (j=0; j<kc && r<=nb_rounds; )
  {
    for (; j < kc && t < 4; j++, t++)
      *((uint4 *)&W[r][t]) = *((uint4 *)&tk[j]);
  ...
  while (r <= nb_rounds) // while not enough round key material calculated
  {
    ...
    if (kc != 8)  // 128- and 192-bit keys
    {
      for (j = 1; j < kc; j++)
        *((uint4 *)&tk[j]) ^= *((uint4 *)&tk[j-1]);
    }
    else          // 256-bit keys
    {
      for (j=1; j<(kc>>1); j++)
      ...
      tk[kc>>1][0] ^= S[tk[(kc>>1) - 1][0]];
      tk[kc>>1][1] ^= S[tk[(kc>>1) - 1][1]];
      tk[kc>>1][2] ^= S[tk[(kc>>1) - 1][2]];
      tk[kc>>1][3] ^= S[tk[(kc>>1) - 1][3]];
...
void fast_conv_key_for_dec (ref KEY k)
{
  ref byte W[MAXROUNDS+1][4][4] = k.rk;
  int     r, j;
  byte[4] *a;

  for (r=1; r<k.nb_rounds; r++)
  {
    a = &W[r];
    for (j=0; j<4; j++)
      *((uint4 *)&a[j]) = *((uint4 *)&U1[a[j][0]]) ^ *((uint4 *)&U2[a[j][1]])
                        ^ *((uint4 *)&U3[a[j][2]]) ^ *((uint4 *)&U4[a[j][3]]);
  }
}

//-------------------------------------------------------------------------------

void fast_encrypt (    byte a[16],
                   out byte b[16],
                       byte rk[MAXROUNDS+1][4][4],
                       int  nb_rounds)
{
  ...
  *((uint4*)&temp[0]) = *((uint4*)&a[0])  ^ *((uint4*)&rk[0][0]);
  *((uint4*)&temp[1]) = *((uint4*)&a[4])  ^ *((uint4*)&rk[0][1]);
  *((uint4*)&temp[2]) = *((uint4*)&a[8])  ^ *((uint4*)&rk[0][2]);
  *((uint4*)&temp[3]) = *((uint4*)&a[12]) ^ *((uint4*)&rk[0][3]);

  clear b;

  *((uint4*)&b[0]) = *((uint4*)&T1[temp[0][0]])
                   ^ *((uint4*)&T2[temp[1][1]])
                   ^ *((uint4*)&T3[temp[2][2]])
                   ^ *((uint4*)&T4[temp[3][3]]);
  *((uint4*)&b[4]) = *((uint4*)&T1[temp[1][0]])
                    ^ *((uint4*)&T2[temp[2][1]])
                    ^ *((uint4*)&T3[temp[3][2]])
                    ^ *((uint4*)&T4[temp[0][3]]);
  *((uint4*)&b[8]) = *((uint4*)&T1[temp[2][0]])
                    ^ *((uint4*)&T2[temp[3][1]])
                    ^ *((uint4*)&T3[temp[0][2]])
                    ^ *((uint4*)&T4[temp[1][3]]);
  *((uint4*)&b[12]) = *((uint4*)&T1[temp[3][0]])
                     ^ *((uint4*)&T2[temp[0][1]])
                     ^ *((uint4*)&T3[temp[1][2]])
                     ^ *((uint4*)&T4[temp[2][3]]);

  for (r = 1; r < nb_rounds-1; r++)
  {
    *((uint4*)&temp[0]) = *((uint4*)&b[0])  ^ *((uint4*)&rk[r][0]);
    *((uint4*)&temp[1]) = *((uint4*)&b[4])  ^ *((uint4*)&rk[r][1]);
    *((uint4*)&temp[2]) = *((uint4*)&b[8])  ^ *((uint4*)&rk[r][2]);
    *((uint4*)&temp[3]) = *((uint4*)&b[12]) ^ *((uint4*)&rk[r][3]);
  ...
  *((uint4*)&temp[0]) = *((uint4*)&b[0])  ^ *((uint4*)&rk[nb_rounds-1][0]);
  *((uint4*)&temp[1]) = *((uint4*)&b[4])  ^ *((uint4*)&rk[nb_rounds-1][1]);
  *((uint4*)&temp[2]) = *((uint4*)&b[8])  ^ *((uint4*)&rk[nb_rounds-1][2]);
  *((uint4*)&temp[3]) = *((uint4*)&b[12]) ^ *((uint4*)&rk[nb_rounds-1][3]);
  ...
  *((uint4*)&b[0])  ^= *((uint4*)&rk[nb_rounds][0]);
  *((uint4*)&b[4])  ^= *((uint4*)&rk[nb_rounds][1]);
  *((uint4*)&b[8])  ^= *((uint4*)&rk[nb_rounds][2]);
  *((uint4*)&b[12]) ^= *((uint4*)&rk[nb_rounds][3]);
}

//-------------------------------------------------------------------------------

void fast_decrypt (    byte a[16],
                   out byte b[16],
                       byte rk[MAXROUNDS+1][4][4],
                       int  nb_rounds)
{
  ...
  *((uint4*)&temp[0]) = *((uint4*)&a[0])  ^ *((uint4*)&rk[nb_rounds][0]);
  *((uint4*)&temp[1]) = *((uint4*)&a[4])  ^ *((uint4*)&rk[nb_rounds][1]);
  *((uint4*)&temp[2]) = *((uint4*)&a[8])  ^ *((uint4*)&rk[nb_rounds][2]);
  *((uint4*)&temp[3]) = *((uint4*)&a[12]) ^ *((uint4*)&rk[nb_rounds][3]);

  clear b;

  *((uint4*)&b[0]) = *((uint4*)&T5[temp[0][0]])
                   ^ *((uint4*)&T6[temp[3][1]])
                   ^ *((uint4*)&T7[temp[2][2]])
                   ^ *((uint4*)&T8[temp[1][3]]);
  *((uint4*)&b[4]) = *((uint4*)&T5[temp[1][0]])
                   ^ *((uint4*)&T6[temp[0][1]])
                   ^ *((uint4*)&T7[temp[3][2]])
                   ^ *((uint4*)&T8[temp[2][3]]);
  *((uint4*)&b[8]) = *((uint4*)&T5[temp[2][0]])
                   ^ *((uint4*)&T6[temp[1][1]])
                   ^ *((uint4*)&T7[temp[0][2]])
                   ^ *((uint4*)&T8[temp[3][3]]);
  *((uint4*)&b[12]) = *((uint4*)&T5[temp[3][0]])
                    ^ *((uint4*)&T6[temp[2][1]])
                    ^ *((uint4*)&T7[temp[1][2]])
                    ^ *((uint4*)&T8[temp[0][3]]);
  ...
  // last round is special
  *((uint4*)&temp[0]) = *((uint4*)&b[0])  ^ *((uint4*)&rk[1][0]);
  *((uint4*)&temp[1]) = *((uint4*)&b[4])  ^ *((uint4*)&rk[1][1]);
  *((uint4*)&temp[2]) = *((uint4*)&b[8])  ^ *((uint4*)&rk[1][2]);
  *((uint4*)&temp[3]) = *((uint4*)&b[12]) ^ *((uint4*)&rk[1][3]);
  ...
  *((uint4*)&b[0])  ^= *((uint4*)&rk[0][0]);
  *((uint4*)&b[4])  ^= *((uint4*)&rk[0][1]);
  *((uint4*)&b[8])  ^= *((uint4*)&rk[0][2]);
  *((uint4*)&b[12]) ^= *((uint4*)&rk[0][3]);
}
...
public void aes_encrypt (out byte[16] data_out,
                         byte[16]     data_in,
                         byte[]       key)       // key'length must be 16, 24 or 32.
{
  KEY k;
  fast_compute_key (key, out k);
  fast_encrypt (data_in, out data_out, k.rk, k.nb_rounds);
}

//-------------------------------------------------------------------------------

public void aes_decrypt (out byte[16] data_out,
                         byte[16]     data_in,
                         byte[]       key)       // key'length must be 16, 24 or 32.
{
  KEY k;
  fast_compute_key (key, out k);
  fast_conv_key_for_dec (ref k);
  fast_decrypt (data_in, out data_out, k.rk, k.nb_rounds);
}

//-------------------------------------------------------------------------------

public void aes_mac (out byte[] mac,    // mac'length must be in range 1 .. 16
                     ...
                     byte[]     key)    // key'length must be 16, 24 or 32.
{
  ...
  while (length >= 16)
  {
    ...
    fast_encrypt (cipher, out cipher, k.rk, k.nb_rounds);
...
public void aes_init_encrypt (out AES_ENCRYPT_KEY aes, byte[] key)
{
  clear aes;
  fast_compute_key (key, out aes.key);
}

//--------------------------------------------------------------------------

// key'length must be 16, 24 or 32.
public void aes_init_decrypt (out AES_DECRYPT_KEY aes, byte[] key)
{
  clear aes;
  fast_compute_key (key, out aes.key);
  fast_conv_key_for_dec (ref aes.key);
}

//--------------------------------------------------------------------------

public void aes_fast_encrypt (    AES_ENCRYPT_KEY  aes,
                                  byte[16]         data_in,
                              out byte[16]         data_out)
{
  fast_encrypt (data_in, out data_out, aes.key.rk, aes.key.nb_rounds);
}

//--------------------------------------------------------------------------

public void aes_fast_decrypt (    AES_DECRYPT_KEY  aes,
                                  byte[16]         data_in,
                              out byte[16]         data_out)
{
  fast_decrypt (data_in, out data_out, aes.key.rk, aes.key.nb_rounds);
}
...
public void crypt_block (    byte[]    key,      // key'length must be 16, 24 or 32.
                         ...
                         ref byte[]    data)
{
  ...
  while (length >= 16)
  {
    ...
    #begin unsafe
    *((uint4*)&data[ofs]) ^= *((uint4*)&cipher[0]);
    *((uint4*)&data[ofs+4]) ^= *((uint4*)&cipher[4]);
    *((uint4*)&data[ofs+8]) ^= *((uint4*)&cipher[8]);
    *((uint4*)&data[ofs+12]) ^= *((uint4*)&cipher[12]);
    #end unsafe
