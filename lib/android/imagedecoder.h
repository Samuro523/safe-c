...
// imagedecoder.h : ImageDecoder Android Image Decoder
 ...
 *
 * Similar to the Java counterpart android.graphics.ImageDecoder, it can be used to decode images
 * in the following formats: JPEG, PNG, GIF, WebP, BMP, ICO, WBMP, HEIF, Digital negatives (via the DNG SDK)
 * It has similar options for scaling, cropping, and choosing the output format.
 * Unlike the Java API, which can create an android.graphics.Bitmap or android.graphics.drawable.Drawable object,
 * AImageDecoder decodes directly into memory provided by the client. 
 * For more information, see the <a href="https://developer.android.com/ndk/guides/image-decoder">Image decoder</a> 
 */

typedef byte AImageDecoder;
typedef int2 AImageDecoderHeaderInfo;
typedef int int32_t;
typedef uint size_t;
     ...
     */
const int ANDROID_IMAGE_DECODER_SUCCESS = 0;
    /**
     * The input is incomplete.
     */
const int ANDROID_IMAGE_DECODER_INCOMPLETE = -1;
    /**
     * The input contained an error after decoding some lines.
     */
const int ANDROID_IMAGE_DECODER_ERROR = -2;
    /**
     * Could not convert. For example; attempting to decode an image with
     * alpha to an opaque format.
     */
const int ANDROID_IMAGE_DECODER_INVALID_CONVERSION = -3;
    /**
     * The scale is invalid. It may have overflowed; or it may be incompatible
     * with the current alpha setting.
     */
const int ANDROID_IMAGE_DECODER_INVALID_SCALE = -4;
    /**
     * Some other parameter is invalid.
     */
const int ANDROID_IMAGE_DECODER_BAD_PARAMETER = -5;
    /**
     * Input was invalid before decoding any pixels.
     */
const int ANDROID_IMAGE_DECODER_INVALID_INPUT = -6;
    /**
     * A seek was required and it failed.
     */
const int ANDROID_IMAGE_DECODER_SEEK_ERROR = -7;
    /**
     * Some other error. For example; an internal allocation failed.
     */
const int ANDROID_IMAGE_DECODER_INTERNAL_ERROR = -8;
    /**
     * AImageDecoder did not recognize the format.
     */
const int ANDROID_IMAGE_DECODER_UNSUPPORTED_FORMAT = -9;
    /**
     * The animation has reached the end.
     */
const int ANDROID_IMAGE_DECODER_FINISHED = -10;
    /**
     ...
     */
const int  ANDROID_IMAGE_DECODER_INVALID_STATE = -11;
...
[extern "libjnigraphics.so"]
int AImageDecoder_createFromAAsset (long asset, AImageDecoder** outDecoder);
...
[extern "libjnigraphics.so"]
int AImageDecoder_setAndroidBitmapFormat (AImageDecoder* decoder, int format);
...
[extern "libjnigraphics.so"]
AImageDecoderHeaderInfo*  AImageDecoder_getHeaderInfo (AImageDecoder* decoder);
 ...
 */
[extern "libjnigraphics.so"]
int32_t AImageDecoderHeaderInfo_getWidth(AImageDecoderHeaderInfo* _Nonnull)
       ;
 ...
 */
[extern "libjnigraphics.so"]
int32_t AImageDecoderHeaderInfo_getHeight(AImageDecoderHeaderInfo* _Nonnull)
       ;
 ...
 */
char* _Nullable AImageDecoder_resultToString(int)__INTRODUCED_IN(31);
 ...
 *   supported.
 */
int AImageDecoder_createFromFd(int fd, AImageDecoder* _Nullable * outDecoder)
       ;
 ...
 *   supported.
 */
int AImageDecoder_createFromBuffer(void* buffer, size_t length,
                                   AImageDecoder* _Nullable * outDecoder)
       ;
 ...
 */
int AImageDecoder_computeSampledSize(AImageDecoder* decoder, int sampleSize,
 ...
 */
char*  AImageDecoderHeaderInfo_getMimeType(
        AImageDecoderHeaderInfo* _Nonnull);
...
int AImageDecoder_getFrameInfo(AImageDecoder* decoder,
        AImageDecoderFrameInfo* info) __INTRODUCED_IN(31);
