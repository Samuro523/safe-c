...
struct  my_marker_writer {
  jpeg_marker_writer pub; /* public fields */
  uint last_restart_interval; /* last DRI value emitted; 0 after SOI */
}
...
/* Returns the precision used (0 = 8bits, 1 = 16bits) for baseline checking */
{
  ...
  if (qtbl == null)
    abort; // cERREXIT1((j_compress_ptr)cinfo, JERR_NO_QUANT_TABLE, index);
...
/* Emit a DHT marker */
{
  ...
  if (is_ac) {
    htbl = cinfo->ac_huff_tbl_ptrs[index2];
    index2 += 0x10;		/* output index has AC bit set */
  } else {
    htbl = cinfo->dc_huff_tbl_ptrs[index2];
  }

  if (htbl == null)
    abort; // cERREXIT1((j_compress_ptr)cinfo, JERR_NO_HUFF_TABLE, index2);
...
/* Emit a SOF marker */
{
  ...
  if ((uint) cinfo->image_height > 65535 ||
      (uint) cinfo->image_width > 65535)
    abort; // cERREXIT1((j_compress_ptr)cinfo, JERR_IMAGE_TOO_BIG, 65535);
...
void
write_marker_header (j_compress_ptr cinfo, int marker, uint datalen)
/* Emit an arbitrary marker header */
{
  if (datalen > 65533)		/* safety check */
    abort; // cERREXIT(cinfo, JERR_BAD_LENGTH);
...
jinit_marker_writer (j_compress_ptr cinfo)
{
  ...
  marker = (my_marker_ptr) cinfo->mem->alloc_small ((j_common_ptr) cinfo, JPOOL_IMAGE, (my_marker_writer'size));
  cinfo->marker = (jpeg_marker_writer *) marker;
